...
float checkers(vec3 uv) {
    float repeats = 2.0;
    float cx = floor(repeats * uv.x);
    float cy = floor(repeats * uv.y);
    float cz = floor(repeats * uv.z);
...
void main() {
    ...
    */
    ...
    // Color = vec4(fract(position), time);

    float distFromEdge = snoise(position) * 0.5 + 0.5;
    ...
    Color = vec4(
        mix(vec3(0.0), vec3(1.0), distFromEdge),
        1.0
    );
}
